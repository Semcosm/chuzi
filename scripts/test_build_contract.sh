...
[ -f "$repo_root/launcher-ui/Cargo.toml" ] || fail "launcher UI Cargo manifest is missing"
[ -f "$repo_root/launcher-ui/Cargo.lock" ] || fail "launcher UI Cargo lockfile is missing"
...
[ -f "$repo_root/launcher-ui/src/config.rs" ] || fail "launcher UI config boundary is missing"
[ -f "$repo_root/launcher-ui/src/launcher_process.rs" ] || fail "launcher UI process boundary is missing"
[ -f "$repo_root/launcher-ui/frontend/src/index.html" ] || fail "launcher UI document is missing"
[ -f "$repo_root/launcher-ui/frontend/src/app.ts" ] || fail "launcher UI frontend entrypoint is missing"
...
grep -Fq 'refreshInFlight' "$repo_root/launcher-ui/frontend/src/features/launcher-controller.ts" || fail "launcher UI does not serialize refreshes"
grep -Fq 'launcher_request' "$repo_root/launcher-ui/frontend/src/services/ipc.ts" || fail "launcher UI IPC service is missing"
grep -Fq 'node - "$repo_root/launcher-ui/frontend/src/index.html"' "$repo_root/scripts/test_launcher_ui.sh" || fail "launcher UI module harness is missing"
...
grep -Fq 'Test native desktop UI and WebView helper contract' "$repo_root/.github/workflows/chuzi-build.yml" || fail "workflow misses desktop helper contract"
...
grep -Fq 'launcher-ui/Cargo.toml' "$repo_root/scripts/build.sh" || fail "build.sh misses launcher UI"
...
grep -Fq 'chuzi-launcher-ui' "$repo_root/scripts/generate_release_manifest.py" || fail "release manifest misses launcher UI"
