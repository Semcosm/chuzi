...
if (-not [string]::IsNullOrWhiteSpace($msbuild) -and (Test-Path $msbuild)) {
    & $msbuild $project /restore /t:Publish "/p:Configuration=$Configuration" "/p:Platform=x64" "/p:TargetFramework=net8.0-windows10.0.19041.0" "/p:RuntimeIdentifier=win-x64" "/p:SelfContained=true" "/p:PublishSingleFile=false" "/p:PublishDir=$publishDir"
} else {
    & dotnet publish $project --configuration $Configuration --framework net8.0-windows10.0.19041.0 --runtime win-x64 --self-contained true -p:PublishSingleFile=false --output $publishDir
}
