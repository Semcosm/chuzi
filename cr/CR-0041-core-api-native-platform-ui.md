...
The branch also carries the runtime, adapter, release, and parallel CI
prerequisites needed to validate this boundary. These prerequisites do not
change the ownership model: Core remains the only client-facing control plane,
while storage, credentials, and browser/runtime internals stay behind the
service.
