...
| 业务运行时 | 尚未实现 | 状态存储、队列、凭证、Matrix 和真实浏览器仍未接入 |
...
实现 `internal/account`，并补充账号状态机的实现级定义：事件 ID、重复
事件幂等规则、非法转换错误、重试次数和退避、租约超时、服务重启恢复、
操作者与组件身份以及可审计事件格式。
...
默认方向是先支持单节点部署，并保持 `CGO_ENABLED=0` 的跨平台构建；若
目标变为多实例部署，应在本阶段明确外部数据库、并发一致性和迁移策略，
不能把单机文件数据库的假设带入集群运行。
...
### 阶段五：Session Runner 与 Worker 生命周期

状态：规划中，依赖阶段一至四的接口。

扩展现有 Worker 协议和 `internal/browser`，实现每账号独立 Profile、路径
生成、互斥租约、Worker 启停、心跳、超时和崩溃回收。先使用 fake Worker
完成 Go 控制面集成，不在本阶段直接引入真实浏览器下载。
...
- fake Worker 集成测试覆盖正常退出、崩溃、超时和重复事件。
