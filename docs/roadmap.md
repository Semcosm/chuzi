...
组件/插件和行为设置先以 `internal/launcher` 接口固定。CR-0022 已实现 UI 无关的
本地更新检查、资源修复、组件依赖启停、插件归档安装和显式签名者信任；网络下载、
生产签名验证、插件执行和行为设置持久化仍需后续独立 CR。
...
状态：已完成（CR-0023，稳定版签名发布除外）。

生产运行拼装已提供：`configs/example.json`、`deploy/`、健康检查、Matrix 网络与
通知 worker、凭证注入、数据库备份恢复和服务入口接线均已完成。日志轮转、指标、
审计查询以及稳定版签名发布仍是后续增量；当前构建/打包脚本继续只生成 nightly
产物和 SHA256 校验文件。
